  ...
  """
    ...
    def load(
        ...
        **kwargs
        ...
        lob_set.data_dict[50] = [y]
        lob_set.data_dict[10] = [y]
        ...
        len_per_day_fake_list = [int(number)] * 9
        len_per_day_fake_list.append(len(y) - np.sum(len_per_day_fake_list))

        lob_set.properties["LEN_PER_DAY_PER_STOCK"] = [len_per_day_fake_list]

        # LEN_PER_DAY_PER_STOCK, features, targets, _ordered_indices

        ## PRODIGY AI HOCKUS POCKUS START END
...
class Extract(Layer):
    ...
    def _link(self, x, **kwargs):
        ...
        if not self._volume_only and self._max_level == 10:
            return x
        # Apply max_level
