...
class SequenceSet(DataSet):
  ...
  """
    ...
    def targets(self, val):
        if val is not None:
            if isinstance(val, list):
                ...
                raise TypeError("!! Unsupported target type {}".format(type(val)))
    ...
    @property
    def structure(self):
        return [len(s) for s in self.representative]
    ...
    def merged_data_dict(self):
        ...
        for name, summ_list in self.summ_dict.items():
            ...
            for summ, seq_len in zip(summ_list, self.structure):
                if np.isscalar(summ):
                    summ = np.array([summ]).reshape(1, 1)
    ...
    def __getitem__(self, item):
        if isinstance(item, str):
            ...
            elif item in self.summ_dict.keys():
                return self.summ_dict[item]
        ...
        data_set = SequenceSet(
            data_dict=self._apply(f),
            summ_dict=self._apply(f, self.summ_dict),
    ...
    def get_round_length(self, batch_size, num_steps=None, training=False):
        if num_steps is None:
            ...
            if training and th.sample_among_sequences:
                ...
                round_len = sum(
                    [
                        np.ceil(L / (num_steps if num_steps > 0 else L))
        ...
        if training:
            self._set_dynamic_round_len(round_len)
        return round_len

    def gen_rnn_batches(
        self, batch_size=1, num_steps=-1, shuffle=False, is_training=False, **kwargs
    ):
    ...
    """
        ...
        if is_training and hasattr(self, self.RNN_BATCH_GENERATOR):
            ...
            _gen_rnn_batches = self._gen_rnn_batches_traversal
        # Yield
    ...
    def _gen_rnn_batches_traversal(self, batch_size, num_steps, is_training, round_len):
    ...
    def _gen_rnn_batches_by_wheel(self, batch_size, num_steps, round_len, L, **_):
   ...
   """
        ...
        for name, seq_list in self.data_dict.items():
            ...
            if not isinstance(seq_list, list) or len(seq_list) != list_length:
                raise ValueError(
                    "!! {} should be a list with length {}".format(name, list_length)
                )
            # Check structure
            if [len(s) for s in seq_list] != self.structure:
                raise ValueError(
                    "!! sequence list structure inconformity: {} ".format(name)
                )
            ...
            if shapes.count(shapes[0]) != len(shapes):
                raise AssertionError(
                    "!! Sample shape in {} are inconformity".format(name)
                )
        ...
        for name, summ_list in self.summ_dict.items():
            ...
            if not isinstance(summ_list, list) or len(summ_list) != list_length:
                raise ValueError(
                    "!! {} should be a list of length {}".format(name, list_length)
                )

            if checker.check_scalar_list(summ_list):
                continue
            ...
            for i, summ in enumerate(summ_list):
                if summ.shape[0] > 1:
                    summ_list[i] = np.reshape(summ, (1, *summ.shape))
    ...
    def _get_pe_round_length(self, batch_size, num_steps):
        if self.batch_preprocessor is not None and self.length_calculator is None:
            return 10000  # TODO: default round length
        if self.batch_preprocessor is None:
            assert self.length_calculator is None
    ...
    def _gen_parallel_batches(self, batch_size, num_steps, shuffle):
       ...
       are required"""
        # Sanity check
        if not self.parallel_on:
            raise ValueError("!! parallel engine is not activated")
        ...
        while True:
            ...
            if len(reset_indices) > 0:
                ...
                data_batch.reset_values = (
                    reset_values
                    if len([val for val in reset_values if val is None]) > 0
                    else None
                )
