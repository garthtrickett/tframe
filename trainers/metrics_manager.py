...
class MetricsManager(object):
    ...
    def __init__(self, model):
        ...
        for i in reversed(range(hub.lives)):
            self._RAR[i] = []
    ...
    @property
    def eval_slot(self):
        if self._eval_metric_slot is None:
            return self.early_stop_slot
        else:
            return self._eval_metric_slot
    ...
    @property
    def RAR_string(self):
        self.rar0 = 1  # GARTH HOKUS POKUS HACK NOT SURE WHATS GOING ON HERE
        ...
        rs = " => ".join(
            [
                "L{}({}){}".format(
                    lv,
                    len(scalars),
                    "->{:.3f}".format(scalars[-1]) if len(scalars) > 0 else "",
                )
                for lv, scalars in self._RAR.items()
            ]
        )
    ...
    def get_slot_by_name(self, name):
        result = [s for s in self.metrics if s.name == name or s.symbol.lower() == name]
        if len(result) == 0:
    ...
    def idle_counter(self, slot, rnd):
        ...
        if self.trainer.is_online:
            ...
            return slot.get_idle_rounds(rnd)
    ...
    """
        ...
        for slot, scalar in slot_scalar_dict.items():
            ...
            if slot not in od.keys():
                od[slot] = Statistic(max_length=2)
            stat = od[slot]
            ...
            if take_down_on_slot:
                ...
                new_record = slot.take_down(
                    scalar, rnd, self.model.counter, hub.record_gap
                )
